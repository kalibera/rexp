...
SEXP attribute_hidden do_dotcall(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	for(i = 0; constsOK && i < nargs; i++)
            /* 7: not numerical comparison, not single NA, not attributes as set,
                do ignore byte-code, do ignore environments of closures */
            if (!R_compute_identical(cargs[i], cargscp[i], 7)
		    && !R_checkConstants(FALSE))
		constsOK = FALSE;
	if (!constsOK) {
	    REprintf("ERROR: detected modification of compiler constants after .Call"
		" invocation of function %s.\n", buf);
	    for(i = 0; i < nargs; i++)
		if (!R_compute_identical(cargs[i], cargscp[i], 7))
