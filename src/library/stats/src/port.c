...
{
    SEXP gval; VAPROTECT(gval, coerceVector(eval(gr, rho), REALSXP));
    if (LENGTH(gval) != n)
    ...
    if (hv) {
	SEXP hval; VAPROTECT(hval, eval(hs, rho));
...
{
    SEXP val; VAPROTECT(val, eval(gf, rho));
...
{
    SEXP v; VAPROTECT(v, eval(fcn, rho));
