...
static void Init_R_Machine(SEXP rho)
{
    ...
    VAPROTECT(ans, allocVector(VECSXP, 18));
    VAPROTECT(nms, allocVector(STRSXP, 18));
...
static void Init_R_Platform(SEXP rho)
{
    ...
    VAPROTECT(value, allocVector(VECSXP, 8));
    VAPROTECT(names, allocVector(STRSXP, 8));
...
SEXP attribute_hidden do_filerename(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    VAPROTECT(ans, allocVector(LGLSXP, n1));
    for (i = 0; i < n1; i++) {
...
SEXP attribute_hidden do_fileinfo(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    VAPROTECT(ans, allocVector(VECSXP, ncols));
    VAPROTECT(ansnames, allocVector(STRSXP, ncols));
...
# endif
    ...
    VAPROTECT(xxclass, mkString("octmode"));
...
SEXP attribute_hidden do_localeconv(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    VAPROTECT(ans, allocVector(STRSXP, 18));
    VAPROTECT(ansnames, allocVector(STRSXP, 18));
...
# endif
    ...
    VAPROTECT(ans, allocVector(LGLSXP, 16));
    VAPROTECT(ansnames, allocVector(STRSXP, 16));
...
SEXP attribute_hidden do_filecopy(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    fn = CAR(args);
    nfiles = length(fn);
    VAPROTECT(ans, allocVector(LGLSXP, nfiles));
    if (nfiles > 0) {
	args = CDR(args);
	if (!isString(fn))
	    error(_("invalid '%s' argument"), "from");
...
SEXP attribute_hidden do_l10n_info(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    VAPROTECT(names, allocVector(STRSXP, len));
...
SEXP attribute_hidden do_syschmod(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    n = LENGTH(paths);
    VAPROTECT(smode, coerceVector(CADR(args), INTSXP));
    modes = INTEGER(smode);
    m = LENGTH(smode);
...
SEXP attribute_hidden do_sysumask(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    R_Visible = FALSE;
#endif
    VAPROTECT(ans, ScalarInteger(res));
...
SEXP attribute_hidden do_Cstack_info(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    checkArity(op, args);
    VAPROTECT(ans, allocVector(INTSXP, 4));
    VAPROTECT(nms, allocVector(STRSXP, 4));
