...
SEXP attribute_hidden do_subset(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if(R_DispatchOrEvalSP(call, op, R_SubsetChar, args, rho, &ans)) {
...
SEXP attribute_hidden do_subset2(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if(R_DispatchOrEvalSP(call, op, R_Subset2Char, args, rho, &ans)) {
	if (NAMED(ans))
	    SET_NAMED(ans, 2);
	return(ans);
    }
...
SEXP attribute_hidden do_subset3(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if(R_DispatchOrEvalSP(call, op, R_DollarChar, args, env, &ans)) {
	UNPROTECT(2);
	if (NAMED(ans))
	    SET_NAMED(ans, 2);
	return(ans);
