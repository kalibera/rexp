...
SEXP attribute_hidden do_isunsorted(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    SEXP ans, x = CAR(args);
    if(DispatchOrEval(call, op, R_IsUnsortedCharSXP, args, rho, &ans, 0, 1))
	return ans;
...
SEXP attribute_hidden do_xtfrm(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if(DispatchOrEval(call, op, R_XtfrmCharSXP, args, rho, &ans, 0, 1)) return ans;
    /* otherwise dispatch the default method */
    PROTECT(fn = findFun(install("xtfrm.default"), rho));
    PROTECT(prargs = PROMISE_ARGS(args, R_GlobalEnv));
    SET_PRVALUE_IF_PROMISE(CAR(prargs), CAR(args));
    ans = applyClosure(call, fn, prargs, rho, R_NilValue);
    UNPROTECT(2);
    RELEASE_PROMARGS(prargs);
    return ans;
