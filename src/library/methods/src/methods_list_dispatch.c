...
static SEXP get_generic(SEXP symbol, SEXP rho, SEXP package)
{
    ...
    if(!isSymbol(symbol))
	symbol = installCharSXP(asChar(symbol));
			...
			int evalArgs)
{
	...
	/* shouldn't happen, since argument in class MethodsList has class
	   "name" */
	arg_sym = installCharSXP(asChar(arg_slot));
...
static SEXP R_loadMethod(SEXP def, SEXP fname, SEXP ev)
{
	...
	else if(t == R_SourceSymbol || t == s_generic /* can we assume that "generic" is always there like "class"? */)  {
	    /* ignore */ found++;
	}
    }
...
SEXP R_getClassFromCache(SEXP class, SEXP table)
{
	...
	value = findVarInFrame(table, installCharSXP(STRING_ELT(class, 0)));
	if(value == R_UnboundValue)
	    return R_NilValue;
