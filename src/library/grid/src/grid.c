		   ...
		   pGEDevDesc dd)
{
	 ...
	 */
	defineVar(installCharSXP(STRING_ELT(VECTOR_ELT(vp, VP_NAME), 0)),
		  vp, 
...
static SEXP findInChildren(SEXP name, SEXP strict, SEXP children, int depth) 
{
	...
	result = findViewport(name, strict,
			      findVar(installCharSXP(STRING_ELT(childnames, count)),
				      children),
			      depth);
...
static SEXP findViewport(SEXP name, SEXP strict, SEXP vp, int depth) 
{
	...
	SET_VECTOR_ELT(result, 1, 
			...
			*/
		       findVar(installCharSXP(STRING_ELT(name, 0)), 
			       viewportChildren(vp)));
    } else {
	/*
				 ...
				 SEXP children, int depth) 
{
	...
	PROTECT(vp = findVar(installCharSXP(STRING_ELT(childnames, count)),
			     children));
